...
growFocused :: Zipper (Elt a) -> Dir -> Rational -> Rational -> Zipper (Elt a)
growFocused Nothing _ _ _ = Nothing
growFocused (Just s) dir a min = Just $ s'
  where (E w r f) = W.focus s
        dns = W.down s
        ups = W.up s
        l = length dns + length ups + 1
        lst | dir == Down = dns
            | dir == Up = ups
        min' = min -- * fi l
        a' = a -- * fi l
        ...
        (lst',realGrowth) = transferZoom lst a'' min'
        s'  | dir == Down = W.Stack fw ups lst'
            | dir == Up = W.Stack fw lst' dns
        fw = E w (r + realGrowth) f


transferZoom :: [Elt a] -> Rational -> Rational -> ([Elt a], Rational)
transferZoom ((E w r f):xs) a min = if r - a > min 
                                    then (E w (r - a) f : xs, a)
                                    else (E w min f : rest, realGrowth)
                                        where (rest, realGrowth) =  transferZoom xs (a - (r - min)) min
transferZoom [] a min = ([], 0)

-- * Messages

data Dir = Down | Up deriving (Eq, Show, Read)
...
data ZoomMessage = Zoom Rational
                 ...
                 | GrowBy Dir Rational
                 -- ^ Grow to a specific direction, taking or giving
                 -- zoom to neighbours
...
growDown :: ZoomMessage
growDown = GrowBy Down growAmount
growUp :: ZoomMessage
growUp = GrowBy Up growAmount
shrinkDown :: ZoomMessage
shrinkDown = GrowBy Down (-growAmount)
shrinkUp :: ZoomMessage
shrinkUp = GrowBy Up (-growAmount)
...
instance (EQF f a, Show a, Read a, Show (f a), Read (f a))
    ...
    doLayout (ZC f zelts) r@(Rectangle _ _ w _) s
        = let elts = W.integrate' zelts
              zelts' = mapZ_ (\a -> fromMaybe (E a 10 False)
    ...
    pureMessage (ZC f zelts) sm | Just (E a r b) <- getFocusZ zelts
        = case fromMessage sm of
            ...
            Just (GrowBy d r') -> Just $ ZC f $ zelts'
              where zelts' = growFocused zelts d r' (1)
